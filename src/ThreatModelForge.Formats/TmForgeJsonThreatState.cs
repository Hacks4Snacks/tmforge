namespace ThreatModelForge.Formats
{
    ...
    /// <summary>
    /// A threat-overlay entry carried on <c>tmforge-json</c>: the durable, author-owned state of one
    /// threat, so edits and manually-authored threats round-trip with the structural model even though
    /// the rest of the (regenerable) threat register is not stored. A rule-derived threat is keyed by
    /// its register id (<c>{targetGuid:N}:{ruleId}</c>) and only needs an entry once it is edited; a
    /// manually-authored threat sets <see cref="Manual"/> and is keyed in the reserved <c>manual:</c> namespace.
    /// </summary>
